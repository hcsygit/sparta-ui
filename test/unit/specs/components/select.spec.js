...
describe('Select', () => {
  ...
  describe('single', () => {
    ...
    //   expect(wrapper.findAll('.sp-option.is--hover').wrappers.length).to.be.equal(1)
    //   await wrapper.find('.sp-select-other-button').trigger('click')

    //   await wrapper.find('.sp-select').trigger('click')
    //   await wrapper.find('.sp-select__input').trigger('focus')

    //   const optionList = wrapper.findAll('.sp-select-list .sp-option')
    //   expect(optionList.filter(item => item.element.style.display!== 'none').length).to.be.equal(5)
    //   await wrapper.find('.sp-select__input').setValue('pig')
    //   await wrapper.find('.sp-select-other-button').trigger('click')
    //   expect(wrapper.vm.val).to.be.equal(3)
    ...
    //   await wrapper.find('.sp-select').trigger('click')
    //   await wrapper.find('.sp-select__input').trigger('focus')
    //   expect(wrapper.find('.sp-option.is--selected').text()).to.be.equal('pig')
    //   await wrapper.find('.sp-select__input').setValue('ca')
    //   await wrapper.find('.sp-select-other-button').trigger('click')
    //   expect(wrapper.vm.val).to.be.equal(3)

    //   await wrapper.find('.sp-select').trigger('click')
    //   await wrapper.find('.sp-select__input').trigger('focus')
    //   await wrapper.find('.sp-select__input').setValue('cat')
    //   await wrapper.find('.sp-select-other-button').trigger('click')
    //   expect(wrapper.vm.val).to.be.equal(1)

    //   await wrapper.find('.sp-select').trigger('click')
    //   await wrapper.find('.sp-select__input').trigger('focus')
    //   await wrapper.findAll('.sp-option').wrappers[1].trigger('click')
    //   expect(wrapper.vm.val).to.be.equal(2)
    // });

    it('filterable', async (done) => {
      // 是看第一次输入错误的值，然后失去焦点，是否会清空数据
      await wrapper.setData({ val: '', filterable: true })
      await wrapper.vm.$el.querySelector('.sp-select').click()
      await wrapper.find('.sp-select__input').setValue('pi')
      await wrapper.vm.$el.querySelector('.sp-select-other-button').click()
      expect(wrapper.vm.val).to.be.equal('')
      expect(wrapper.vm.$children[1].visible).to.be.equal(false)
      expect(wrapper.vm.$children[1].inputText).to.be.equal('')

      // // 是看第一次输入包含的数据，看看过滤功能是否正常
      // await wrapper.vm.$el.querySelector('.sp-select').click()
      // await wrapper.find('.sp-select__input').setValue('pi')
      // let optionList = wrapper.findAll('.sp-select-list .sp-option')
      // expect(optionList.filter(item => item.element.style.display!== 'none').length).to.be.equal(1)

      // const select = wrapper.vm.$children[1];
      // // 过滤成功之后，操作键盘向下
      // await select.navigateOptions('next')
      // expect(wrapper.find('.sp-select-list .is--hover').text()).to.be.equal('pig')
      ...
      // optionList = wrapper.findAll('.sp-select-list .sp-option')
      // expect(optionList.filter(item => item.element.style.display!== 'none').length).to.be.equal(5)
      // await wrapper.find('.sp-select__input').setValue('pig')
      // await wrapper.find('.sp-select-other-button').trigger('click')
      // expect(wrapper.vm.val).to.be.equal(3)
