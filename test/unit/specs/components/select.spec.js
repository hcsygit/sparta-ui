...
describe('Select', () => {
  ...
  describe('keyboard', async () => {
    ...
    it('keyboard operations', async () => {
      ...
      select.navigateOptions('prev');
      select.handleInputEnter()
      expect(wrapper.vm.val).to.deep.equal([false])
  ...
  describe('focusSelectInput', () => {
    const wrapper = mount({
      ...
      <sp-button class="sp-select-other-button">测试</sp-button>
        <sp-select 
          ...
          :filterable="true"
          v-model="val">
          <i slot="prepend" class="sp-icon-file"></i>
          <div slot="center">{{ val }}</div>
          <sp-option 
           v-for="(item, index) in optionsData"
           :key="item.id"
           :label="item.name"
    ...
    it('focusSelectInput', async () => {
      await wrapper.find('.sp-select__input-placeholder').trigger('click')
      expect(wrapper.vm.val).to.be.equal('')
    })
